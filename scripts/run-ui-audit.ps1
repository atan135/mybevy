...
function Resolve-UiAuditStates {
    ...
    if ($StateValue.Trim().Equals("auto", [System.StringComparison]::OrdinalIgnoreCase)) {
        if ($Screen -eq "ui_gallery") {
            return "image_fit,visual_foundation,image_modes,image_tiling,image_atlas,typography,typography_overflow,icons,icon_states,style_scopes,effects,middle,bottom"
        }
        return "initial"
    }

    $valid = @("initial", "visual_foundation", "image_fit", "image_modes", "image_tiling", "image_atlas", "typography", "typography_overflow", "icons", "icon_states", "style_scopes", "effects", "top", "middle", "bottom")
...
function Invoke-UiAuditSelfTest {
    ...
    try {
        ...
        Assert-SelfTest ($tasks[0].states -eq "image_fit,visual_foundation,image_modes,image_tiling,image_atlas,typography,typography_overflow,icons,icon_states,style_scopes,effects,middle,bottom") "ui_gallery auto states"
