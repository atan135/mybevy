...
function Resolve-UiAuditStates {
    ...
    if ($StateValue.Trim().Equals("auto", [System.StringComparison]::OrdinalIgnoreCase)) {
        if ($Screen -eq "ui_gallery") {
            return "image_fit,visual_foundation,middle,bottom"
        }
        return "initial"
    }

    $valid = @("initial", "visual_foundation", "image_fit", "top", "middle", "bottom")
...
function Invoke-UiAuditSelfTest {
    ...
    try {
        ...
        Assert-SelfTest ($tasks[0].states -eq "image_fit,visual_foundation,middle,bottom") "ui_gallery auto states"
