...
impl UiControlKind {
    const fn supports_state(self, state: UiControlState) -> bool {
        use UiControlKind as K;
        use UiControlState as S;
        match self {
            K::Button | K::ImageButton | K::Tab => matches!(
                state,
                S::Normal
                    | S::Hovered
                    | S::Pressed
            ...
            K::Badge => matches!(
                state,
                S::Normal | S::Selected | S::Disabled | S::Loading | S::Empty | S::Error
            ),
            K::Progress => matches!(
                state,
                S::Normal | S::Disabled | S::Loading | S::Empty | S::Error
            ),
            K::Tooltip => matches!(state, S::Normal | S::Disabled | S::Error),
            K::Dropdown => true,
            K::Checkbox | K::Toggle | K::Segmented => !matches!(state, S::Empty),
            K::Slider | K::Stepper => matches!(
                state,
                S::Normal | S::Hovered | S::Pressed | S::Focused | S::Disabled | S::Error
            ),
            K::Scroll | K::Modal => matches!(state, S::Normal | S::Loading | S::Empty | S::Error),
        }
    }
}
