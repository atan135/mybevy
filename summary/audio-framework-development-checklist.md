...
- [x] 16. 支持 catalog 外部配置预留
  - 开始时间：2026-06-18 22:14:11 +08:00
  - 结束时间：2026-06-18 22:42:42 +08:00
  - [x] 设计 RON 或 JSON catalog 数据结构。
  - [x] 支持从首包资源读取 catalog 的解析函数。
  - [x] 保留 `content_cache://...` 路径字段兼容。
  - [x] 校验路径不能包含绝对路径、反斜杠、盘符或 `..`。
  - [x] 解析失败时保留内置 fallback catalog。
  - [x] 编写 catalog 配置解析测试。

- [x] 17. 增加调试和诊断能力
  - 开始时间：2026-06-18 22:44:14 +08:00
  - 结束时间：2026-06-18 23:00:08 +08:00
  - [x] 定义 `AudioDebugConfig`。
  - [x] 支持 `MYBEVY_AUDIO_DEBUG`。
  - [x] 支持记录最近播放 cue 和最近跳过 cue。
  - [x] 支持统计活跃实例数量和按 bus 分组数量。
  - [x] 支持记录最近加载失败资源。
  - [x] 预留 UI debug 面板读取的 snapshot 结构。

- [x] 18. 移动端和后台行为检查
  - 开始时间：2026-06-18 23:01:33 +08:00
  - 结束时间：2026-06-18 23:26:54 +08:00
  - [x] 确认 Android APK 能读取 `project/assets/audio/` 内音频。
  - [x] 确认音频格式在 Android Debug 包中可播放。
  - [x] 确认移动端窗口配置或触控流程下 UI 音效可用。
  - [x] 评估进入后台、息屏或应用暂停时的 bus 暂停策略。
  - [x] 确认同屏音频实例数量有上限或可诊断。

- [x] 19. 测试覆盖
  - 开始时间：2026-06-18 23:28:25 +08:00
  - 结束时间：2026-06-18 23:37:26 +08:00
  - [x] 测试 ID 校验和 Display 行为。
  - [x] 测试 catalog cue 到 clip 解析。
  - [x] 测试 bus 音量、静音和暂停计算。
  - [x] 测试 scope 清理筛选。
  - [x] 测试 cue 冷却和最大并发。
  - [x] 测试音乐状态切换和淡入淡出状态推进。
...
- [x] 20. 文档维护
  - 开始时间：2026-06-18 23:38:57 +08:00
  - 结束时间：2026-06-18 23:53:08 +08:00
  - [x] 更新 `docs/audio/README.md`，把已落地能力和未实现能力区分清楚。
  - [x] 如新增资源目录或格式约定，检查 `docs/assets-workflow.md` 是否需要同步。
  - [x] 如新增启动或调试环境变量，检查 `docs/bevy-getting-started.md` 是否需要同步。
  - [x] 如 audio 和 scene manifest 对接，检查 `docs/scene/` 是否需要补充音频边界。
  - [x] 不把后续下载、空间音频或战斗音频高级能力写成已完成，除非代码已经落地。

- [x] 21. 阶段验收
  - 开始时间：2026-06-18 23:54:06 +08:00
  - 结束时间：2026-06-19 00:00:29 +08:00
  - [x] 执行 `cargo fmt --check`。
  - [x] 执行 `cargo check`。
  - [x] 执行 `cargo test`。
  - [x] 执行 `git diff --check`。
  - [x] 运行桌面客户端，验证 UI 点击音和音乐播放。
