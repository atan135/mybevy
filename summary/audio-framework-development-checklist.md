...
- [ ] 6. 实现内存版 `AudioCatalog`
  - 开始时间：
  - 结束时间：
  - [ ] 支持注册 clip ID 到资源路径的映射。
  - [ ] 支持注册 cue ID 到 clip 或 clip 列表的映射。
  - [ ] cue 支持默认 bus、scope、音量、音高和循环标记。
  - [ ] cue 支持冷却、最大并发和优先级字段的结构定义。
  - [ ] 提供 cue 和 clip 缺失时的明确错误。
  - [ ] 编写 catalog 基础单元测试。

- [ ] 7. 新增基础首包音频资源和资源检查
  - 开始时间：
  - 结束时间：
  - [ ] 新增 `project/assets/audio/` 子目录结构。
  - [ ] 放入最小 UI 点击音和测试音乐资源，或使用明确占位资源策略。
  - [ ] 确认资源命名使用小写英文、数字、下划线或短横线。
  - [ ] 使用 `git check-attr filter -- <path>` 确认二进制音频命中 Git LFS。
  - [ ] 记录资源授权来源或确认只使用可发布资源。
...
- [ ] 11. 接入 UI 音效 adapter
  - 开始时间：
  - 结束时间：
  - [ ] 消费 `UiButtonEvent` 并播放默认点击 cue。
  - [ ] 支持按钮或 action 覆盖 cue 的扩展点。
  - [ ] 为 UI cue 增加短冷却，避免连点无限叠音。
  - [ ] 确认 UI 音效默认走 `Ui` bus。
  - [ ] 确认静音 `Ui` bus 后按钮点击不再发声。

- [ ] 12. 接入场景音频生命周期
  - 开始时间：
  - 结束时间：
  - [ ] 监听 `SceneEvent::Entered`，为目标场景播放基础音乐或环境音。
  - [ ] 监听 `SceneEvent::Exited` 或退出开始事件，停止或淡出 `Scene(session_id)` scope 音频。
  - [ ] 确认重复进入和退出场景不会残留音频实例。
  - [ ] 为样板场景准备最小 ambience 或测试 cue。
  - [ ] 不在 scene framework 中硬编码具体业务音乐规则。

- [ ] 13. 实现空间音频最小能力
  - 开始时间：
  - 结束时间：
  - [ ] 定义 listener 绑定方式，优先支持相机或玩家 Transform。
  - [ ] 定义 `PlaySpatialCue` 或等价命令。
  - [ ] 支持空间音源跟随实体或固定 Transform。
  - [ ] 支持最大距离和基础衰减参数。
  - [ ] 场景退出或实体销毁后清理空间音频实例。
  - [ ] 记录 Bevy 空间音频能力限制，不承诺复杂 HRTF 或混响。

- [ ] 14. 实现战斗音效播放策略
  - 开始时间：
  - 结束时间：
  - [ ] 定义 battle cue 的基础映射和 bus。
  - [ ] 支持 cue 最大并发。
  - [ ] 支持 cue 冷却。
  - [ ] 支持 cue 优先级或跳过策略。
  - [ ] 支持随机变体。
  - [ ] 支持 `Battle(battle_id)` scope 清理。
  - [ ] 编写高频 cue 不无限创建实例的测试。
