...
Android 真机不再属于本清单执行范围。待 02-06 功能与公网准入完成、目标版本部署到公网后，再启动 `07_公网部署后Android真机验收清单.md`。
...
- [x] 五份前置 清单 和本 清单 完成后，按仓库约定移动到对应 `docs/<领域>/清单/` 归档并纳入 Git 提交。（验证：01-05 归档清单均为 0 个未完成项；06 已移动到 `docs/服务端/清单/`。）
