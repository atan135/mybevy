...
本文是“协议约定 + 当前 runner 对接说明”。它是 `docs/界面/界面自动化审计与优化方案.md` 的下层依赖机制之一；UI 自动化审计负责定义 UI 截图、滚动、分析和修复流程，本机制负责提供统一、可审计、可扩展的远程命令通道。
...
本机制也不承担持续实时状态预览。当前页面、玩家属性、场景、网络和性能状态的进程内持续观察，以及未来外部显示端使用的 snapshot / delta 流，由 [实时状态预览与开发者监视器](实时状态预览与开发者监视器.md) 定义。远程控制继续使用离散异步 task；实时显示端不能通过高频创建 `system.status` task 模拟状态流。两者未来可以共享设备身份、鉴权和审计基础设施，但必须使用不同消息类型、权限和限速策略。
...
后续 `docs/界面/界面自动化审计与优化方案.md` 应引用本文档，避免在 UI 文档内重复定义远程命令通道。
