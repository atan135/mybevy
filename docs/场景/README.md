...
- [游戏层场景使用说明](./游戏层场景使用说明.md)：说明 game layer 如何通过场景表 CSV、framework manifest、layout RON、大厅入口和 HUD 接入具体游戏场景，当前覆盖 `sample.dungeon_room`、`arena.robot_sync` 和由 MyServer authority 协调进入的 `world.main`。
...
`docs/资源工作流.md` 已规定后续下载资源由内容清单管理。场景清单应和内容清单配合：
...
- 脚本默认 `-InputModeA manual -InputModeB off`，让 A 端使用 `WASD` 或方向键控制本地机器人，B 端静止观察；需要自动 bot 时可显式传 `-InputModeA bot` 或 `-InputModeB bot`。
