...
- 依赖 `02_方圆玩家共享渲染与主世界多人实体清单.md` 提供玩家实体和注册表。
- 依赖 `03_主世界摄像机与桌面触控视角操控清单.md` 提供可读取的摄像机平面方向与输入仲裁。
- 与 `世界联机_05_服务端主世界4000米权威移动清单.md` 共同冻结 20Hz、4m/s、0..4000 服务端坐标和 2000 米中心偏移。
