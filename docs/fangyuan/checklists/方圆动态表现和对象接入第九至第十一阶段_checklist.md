...
- [ ] VFX recipe、技能视觉蓝图、装备、NPC 和天道生成物都必须可审核、可预算、可降级。
- [ ] 同一 start tick、seed 和输入事件必须生成一致视觉状态。
- [ ] 动态 primitive state 不能成为玩法 Entity；玩法规则和视觉输出保持分层。
- [ ] 规则层永远优先于个性层，降级时必须保留真实范围、方向和危险边界。
- [ ] 装备 socket 是语义锚点，不等同于骨骼动画系统。
- [ ] 天道生成物默认临时，只有满足固化条件才进入区域或纪元档案。
