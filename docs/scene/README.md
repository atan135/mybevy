...
这份文档用于规划 MyBevy 的场景相关能力。当前项目还没有完整场景系统，`project/src/framework/scene/` 只是预留框架边界；本文描述后续实现时应遵守的概念、模块职责、数据流、资源组织和落地顺序。
...
本文不是当前实现说明。凡是还未落地的内容，均作为后续开发约定和设计目标。
...
后续可把 `project/src/framework/scene/` 拆成以下模块：
...
- `plugin.rs`：场景框架插件入口。
- `command.rs`：场景命令定义，例如进入、退出、重载、预加载。
- `event.rs`：场景事件定义，例如加载进度、进入完成、失败。
- `lifecycle.rs`：状态机和切换流程。
- `registry.rs`：场景注册表，维护 `SceneId` 到清单的映射。
- `manifest.rs`：场景清单结构、解析和校验。
- `loading.rs`：资源加载、依赖跟踪和进度统计。
- `root.rs`：场景根实体、层根实体和统一清理标记。
- `camera.rs`：通用相机 rig、相机策略和切换。
- `streaming.rs`：分区流式加载策略。
- `partition.rs`：Zone、Region、Chunk 的元数据和查询。
- `spawn.rs`：出生点、挂点和实体生成辅助。
- `trigger.rs`：触发器、区域事件和交互入口。
- `debug.rs`：场景调试显示和诊断命令。
- `prelude.rs`：对游戏层暴露常用类型。

初期可以先实现少量文件，不必一次拆完。拆分应跟实际复杂度同步增长。
...
MyBevy 建议使用 RON 或 JSON 场景清单。RON 更贴近 Rust 类型，JSON 更适合内容平台。初期推荐 RON，后续内容发布可以从源数据生成 JSON。
...
(
    version: "1",
    scene_id: "dungeon.forest_001",
    display_name: "Forest Dungeon 001",
    kind: "Dungeon",
    coordinate: (
        up_axis: "Y",
        unit: "meter",
    ...
    entry: (
        default_spawn: "player.default",
        camera: "camera.follow_3d",
    ...
    layers: [
        (
            ...
            assets: [
                (
                    id: "scene.forest.base",
                    kind: "gltf_scene",
                    path: "models/scenes/forest_001/base.glb",
                    label: "Scene(0)",
                ),
            ],
        ),
        (
            ...
            assets: [
                (
                    id: "scene.forest.collision",
                    kind: "collision_mesh",
                    path: "scenes/forest_001/collision.ron",
                ),
            ],
        ),
        (
            ...
            assets: [
                (
                    id: "scene.forest.ambience",
                    kind: "audio",
                    path: "audio/ambience/forest_loop.ogg",
                ),
            ],
        ),
    ],
    ...
    triggers: [
        (
            id: "trigger.exit",
            shape: (Box: (half_extents: [1.0, 2.0, 1.0])),
            position: [12.0, 0.0, -4.0],
            event: "scene.exit_requested",
        ),
    ],
)
```
...
7. 给 Touch Ripple 注册一个轻量场景 ID。

验收：

- 能通过命令进入 `touch_ripple.playground`。
...
1. 支持 layer 独立启停。
2. 支持 chunk 元数据。
3. 根据玩家或相机位置加载附近 chunk。
4. 远离 chunk 自动卸载。
5. 调试显示 chunk 状态。

验收：

- 移动穿过 chunk 边界时能加载新内容。
- 远离内容能卸载。
- 动态实体不会被错误卸载。
