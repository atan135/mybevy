...
- [x] 本阶段只做 Prefab 和 Scene Layout 拆分，不提前实现 Chunk、Bake、mesh merge、GPU Instancing、LOD、AOI 或联网同步。（验证：`docs/方圆灵构/方圆对象资源构建与渲染技术路线.md:1608` 明确这些能力不是第四阶段能力；代码改动未引入相关模块）
...
- [x] 文档同步记录第四阶段实际落地边界和后续阶段延后事项。（验证：`docs/方圆灵构/方圆对象资源构建与渲染技术路线.md:443`、`:1608` 和 `docs/世界观/方圆灵构蓝图规则.md:18`、`:531` 已同步）
