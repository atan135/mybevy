...
本 清单 重点处理静态内容的 draw call / Entity 压力、shared mesh + instance data 原型、材质 profile 和透明/发光预算。本阶段不实现动态 VFX、技能规则层、装备 NPC 接入、Chunk / LOD / AOI、发布期 Bake、蓝图缓存和纪元继承。
...
- [x] 不引入 rotation、quaternion、euler、angular_velocity、rotate 或 spin 能力。（验证：`docs/世界观/方圆灵构蓝图规则.md:584` 仍禁止 rotation、quaternion、euler、angular_velocity、rotate、spin；本 清单 改动未新增相关字段）
...
- 开发总结：完成第六至第八阶段渲染规模化边界只读复核，确认本 清单 应先在已审核 runtime primitive / layout / prefab 数据之上推进静态 CPU 合并、静态实例化和统一材质 profile；动态 VFX、技能规则层、Chunk、LOD、AOI、Bake 和缓存继承继续留在后续 清单。阶段 1 未修改业务代码。
...
- [x] 复核方圆技术路线中阶段 6、7、8 的目标、验收、风险和控制方式。（验证：worker 复核 `docs/方圆灵构/方圆对象资源构建与渲染技术路线.md:1657`、`:1688`、`:1719`，确认阶段 6 为静态 CPU 合并且动态玩家/技能/短生命周期 primitive 不进静态合并，阶段 7 为静态实例化且保留 standard/CPU merge fallback，阶段 8 为统一 `FangyuanMaterial` 且透明/不透明分通道和透明预算受控）
...
- [x] 明确本 清单 不处理动态 VFX、技能规则层、Chunk、LOD、AOI、Bake 和缓存继承。（验证：worker 复核 `docs/方圆灵构/方圆对象资源构建与渲染技术路线.md:1747`、`:1828`、`:1861` 和 `docs/世界观/方圆灵构蓝图规则.md:551`，确认 VFX 属阶段 9，Chunk/LOD/AOI 属阶段 12，Bake 属阶段 13，缓存/继承属于后续能力）
...
- [x] 更新 `docs/方圆灵构/方圆对象资源构建与渲染技术路线.md`，记录 CPU merge、static instancing、FangyuanMaterial、fallback 和风险。（验证：`docs/方圆灵构/方圆对象资源构建与渲染技术路线.md:1657`、`:1695`、`:1732`、`:1768` 分别记录阶段 6/7/8 和规模 fallback 验收，包含 CPU merge、static instance prototype、`FangyuanMaterialProfile`、fallback、风险和压力摘要）
