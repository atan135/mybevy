...
目标服务与 APK 部署到批准的公网 endpoint 后，Android 真机最终验收集中在 `summary/07_公网部署后Android真机验收清单.md`。该阶段仍需覆盖公网 mail HTTPS/TLS、`MailNotifyPush` 后的权威刷新、触控与系统返回键、前后台恢复、网络切换/弱网，以及领取结果未知时的 reconciliation；这些真机项目未执行前不得宣称跨端或公网邮件闭环完成。
