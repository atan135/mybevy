...
本文冻结“参考图 -> `UiDocument` -> 正式游戏页面”流程的工程边界。仓库当前已提供 Stage 1-11 `tools/ui-generation/` 独立工具、输入契约、工作目录规划、provider 无关调用协议、图片预处理、结构化视觉分析、确定性页面规划、素材策略、受控 `UiDocument` 生成验证、有限结构修复、独立进程预览、多状态/响应式审计、离线 `generate-fixture` 闭环、显式晋升、离线评测和受限可观测性。`ui-generation` 的在线生成、OCR 和图片生成适配仍未实现，`GenerationMode::Provider` 会 fail-closed；独立的 `ui-visual-audit analyze-ai` 已提供显式 opt-in 的在线视觉审核 adapter。因此 `generate-fixture` 只能证明仓库自有结构化 fixture 的工程链路，不能冒充任意用户参考图的真实在线生成或视觉分析。
...
生成结果中的 Grid、card/frame、文字角色、控件 slot/state、TextInput、高级图片、binding 和响应式字段如何配置，统一查阅 [界面生成式组件配置参考.md](界面生成式组件配置参考.md)。本文继续只定义生成工具、草稿、晋升和正式包之间的工程边界。
...
依赖方向固定为工具和正式游戏分别依赖 runtime-free core；视觉审计只在 `provider-core` feature 下复用生成工具的 provider 协议：

```text
ui-generation -> ui-document-core
ui-visual-audit -> ui-generation(provider-core), ui-document-core
...
阶段 11 的仓库内证据由 `pwsh -NoProfile -File scripts/run-ui-e2e-acceptance.ps1` 生成。脚本为 regular 与 complex 两个仓库自有 fixture 生成新的安全 run ID，复制输入到临时目录后运行 `generate-fixture`、四个默认 audit device (`desktop`、`phone-landscape`、`phone-1080p-landscape`、`tablet-landscape`) standalone audit、多状态 Modal/Loading fixture、exact self-reference integrity compare、受控失败演练与 worktree isolation self-test。桌面 Runner 的固定验收调用使用三个横屏设备（`phone-landscape`、`phone-1080p-landscape`、`tablet-landscape`）；`desktop` 保留在工具默认矩阵中。临时输入在 `finally` 中删除；可复查证据、命令日志和 JSON/Markdown 报告保留在忽略的 `summary/ui-generation/<run>-report/` 和对应 run root。
...
`audit-document` 在一个新的输出目录内调用 feature-gated standalone declarative preview，为 `ui_document_preview × desktop/phone-landscape/phone-1080p-landscape/tablet-landscape × 每个显式 state` 生成 PNG、严格 preview metadata 和 `audit-manifest.json`。`--require-distinct-from-initial` 是 fixture 的显式视觉契约：只对列出的状态检查截图 hash 必须不同于 initial，并将不满足的格子写入 manifest；它不把所有业务状态都误判为必须视觉不同。standalone preview 固定为声明式 Page panel，因此 `UiNode::Modal` 的画面可审计，但 Modal panel 的输入阻断和 owner cleanup 仍由正式 runtime 测试验证。该工具不能路由或加载游戏业务页面，不写入 `summary/` 以外的目录，不构成审批或晋升。底层 standalone preview 新增 `--page-state <closed-state-id>`，仍只在 `ui-document-preview-tool` 非默认 feature 下编译；默认桌面、Android `--lib` 和 `UiFrameworkPlugin` 不包含生成器、provider 或审计工具。

## 受控晋升

`promotion-decisions`、`record-promotion-decisions`、`promotion-plan` 与 `promote` 形成受控晋升链。普通分析、生成、预览和评测命令不得写入 `project/src/`、`project/assets/` 或 approved 目录；普通修复默认只写 staging/worktree。`closed-loop-patch-apply` 是唯一受审批的代码修复例外：它必须绑定精确 fix plan 和 approval，只能修改 allowlist 中的共享 widget、theme 或 framework Rust 文件，并且始终在隔离 worktree 中执行，不能修改任意业务 Rust 或调用者工作树。只有 `promote` 能写入正式 approved 目录，且它不会写入 `project/src/`。
...
Stage 8 通过 feature-gated standalone binary 复用 [界面声明式预览与热更新.md](界面声明式预览与热更新.md) 中现有的安全 source、事务 reload、状态迁移和截图能力，只能由开发工具显式启动，不能把 provider 或 generator 注册进正式 `UiFrameworkPlugin`。预览成功也不会构成晋升批准或改变草稿的不可信状态。

本轮设计负责从参考图生成可验证的 `UiDocument` 及其受控晋升。参考图与渲染结果的视觉相似度判定、差异分区和审核阈值属于本地后续开发计划 `04_界面参考图视觉审核_checklist.md`，不能用“能够预览”替代视觉审核通过。
