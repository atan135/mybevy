...
Popover 根据 anchor 的最终布局矩形定位，优先下方、空间不足时翻转上方，并 clamp 到安全区和 viewport。Dropdown 在 Modal 上方打开时成为最高可聚焦 panel；选择、Escape 或 click-away 关闭后优先恢复 trigger，不会跳到底层任意按钮。Escape Closed 事件以 Panel Manager 的真实 CloseTop 目标为准。详细事件 reason、支持状态和失败边界见 [界面通用组件与交互状态.md](界面通用组件与交互状态.md)。
...
所有轨道受全局 `UiMotionPolicy` 控制。主题热更新会在主题值应用后取消在途轨道；页面切换的 `CloseAllForOwner` 仍立即 despawn，不因动画延迟 owner 清理、焦点释放或输入解除。完整规则见 [界面动画与动态效果.md](界面动画与动态效果.md)。
