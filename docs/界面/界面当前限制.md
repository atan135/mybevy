...
- Android 设备级安全区、触控、字体、图片切片、效果降级和 soft keyboard 仍依赖 `adb` 或真机环境。当前开发环境没有可用 `adb`，正式未验条件见 [界面安全区与视觉预算.md](界面安全区与视觉预算.md)。
...
- 允许临时直接使用的 Bevy 原语必须附加 `UiDirectBevyVisual` marker；完整状态和判定规则见 [界面高保真视觉能力.md](界面高保真视觉能力.md)。
...
- 详细边界和命令见 [界面声明式预览与热更新.md](界面声明式预览与热更新.md)；业务迁移分母、例外和生产更新目标见 [界面声明式业务界面迁移基线.md](界面声明式业务界面迁移基线.md)。
...
- `ui-generation` 的在线 provider/OCR/图片生成适配尚未实现，`GenerationMode::Provider` 会 fail-closed。独立 `ui-visual-audit analyze-ai` 已提供显式 opt-in 的在线视觉审核 adapter，但不生成 `UiDocument`。离线 `generate-fixture` 已能把真实任务、仓库自有参考图、结构化 fixture、生成/修复、预览和 sealed bundle 串成完整运行包，并可经显式 `promote` 进入正式路由；该证据不能被描述为真实线上生成或任意用户参考图的自动视觉分析。
...
- 生成和视觉审计工具属于桌面/CI 开发工具，拥有独立 Cargo 根：`ui-generation -> ui-document-core`，`ui-visual-audit -> ui-generation(provider-core), ui-document-core`；`project` 只依赖 `ui-document-core` facade。两者都不进入 `project` 正式依赖图、不注册进 `UiFrameworkPlugin`，也不由 Android `cargo ndk ... --lib` 构建。
...
- 页面主体规划为声明式 JSON；未知业务 action/binding 必须阻塞晋升，模型不能生成任意 Rust 业务实现。完整设计边界见 [界面参考图生成与正式包边界.md](界面参考图生成与正式包边界.md)。
- 外部 `ui-visual-audit` 已提供 exact compare、normalize/align、diff metrics、区域审计、语义审计、aggregate gate 和显式 opt-in `analyze-ai`；游戏内 runner 不负责视觉 diff。当前 Gallery 对照仍只有共享 capture contract、首批稳定 pair ID 和人工截图复核，尚未完成完整 code-vs-document 自动对照、任意用户参考图的生成链路或真实 Android 远程验收。
