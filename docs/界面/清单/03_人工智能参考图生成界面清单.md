...
- 依赖 `01_界面高保真视觉基础能力清单.md` 提供可复用视觉原语和组件。
- 依赖 `02_界面声明式描述与运行时生成清单.md` 提供 JSON Schema、验证器和运行时预览。
- 可复用现有 UI audit 截图、设备矩阵、metadata 和报告结构，但视觉相似度审核由 `04_界面参考图视觉审核清单.md` 负责。
