...
本文的“热更新”主要指 desktop Debug document preview/reload。独立的生产 `UiUpdateClient` 已提供固定受信 endpoint、签名 release、版本化 cache generation 与原子激活，但实际 Release/Android 启用仍必须由游戏提供 app-private cache root、二进制 trust roots 和显式检查时机；preview/watch 绝不是该生产通道。生产目标、术语和页面迁移边界见 [界面声明式业务界面迁移基线.md](界面声明式业务界面迁移基线.md)。
...
preview 不提供另一套 schema migration。旧版本先按 `界面声明式文档协议.md` 的相邻版本纯数据迁移生成当前模型，再进入相同 validation/reload 流程。迁移工具不能直接覆盖 approved 文件，输出仍需经过批准流程。
...
只有显式 `promote` 流程可以在完整校验、人工批准、目标所有权、许可证和冲突检查后，把 `UiDocument` JSON、授权资源及封闭模板生成的 owner/route/registration 审阅声明写入正式目录。该命令已实现，并要求调用者重复 dry-run plan 的精确 hash；晋升后的页面仍须由游戏层显式 route lifecycle 通过 closed adapter 加载。验收样例 `generated.acceptance_fixture` 使用 `ui_generated_acceptance` 路由；详细约定见 [界面参考图生成与正式包边界.md](界面参考图生成与正式包边界.md)。
