...
- `ui-generation` 的在线生成/OCR/图片生成尚未内置，`GenerationMode::Provider` 会 fail-closed；`ui-visual-audit analyze-ai` 已提供独立、显式 opt-in 的 HTTPS 在线视觉审核 adapter。当前 runner 默认使用关闭分析或离线 fixture/mock，普通 CI 不读取在线凭据。
- 多语言、字体缩放、软键盘、安全区真机矩阵、完整 code-vs-document 自动对照和 CI 夜间全量策略仍是长期扩展。
...
本地审计通过环境变量和窗口参数启动一个一次性审计进程。远程设备、移动端和多 client 审计通过 `docs/调试/远程调试控制机制.md` 描述的 adminapi 任务模型编排；当前仓库内的远程 runner 已有 Mock 和 Http 两种后端，但真实 server / client 执行能力仍是外部依赖。
...
长期推荐以远程调试控制模式作为主通道。UI 审计文档只定义“要执行哪些 UI 审计动作和如何分析结果”；跨设备命令下发、任务状态、错误码、超时、重试、artifact 和安全边界以 `docs/调试/远程调试控制机制.md` 为准。
...
游戏内路由注册包含完整 canonical screen；PowerShell runner 的 `KnownScreens` 覆盖可自动化审计页面子集。`ai_login_reference` 目前是游戏开发页路由，未加入 runner `KnownScreens`，不能写入普通 runner 矩阵。

| canonical | 游戏 aliases | runner aliases |
| --- | --- | --- |
| `login` | `login` | `login` |
| `character_select` | `character_select`, `character-select`, `characters`, `select_character`, `select-character` | 同左 |
| `lobby` | `lobby`, `game_list`, `game-list`, `list` | 同左 |
| `main_world` | `main_world`, `main-world` | 同左 |
| `audio_settings` | `audio_settings`, `audio-settings`, `audio`, `settings` | 同左 |
| `audio_monitor` | `audio_monitor`, `audio-monitor`, `audio_debug`, `audio-debug` | 同左 |
| `audio_gallery` | `audio_gallery`, `audio-gallery` | 同左 |
| `wanfa_touch_ripple` | `wanfa_touch_ripple`, `wanfa-touch-ripple`, `touch`, `touch_ripple`, `touch-ripple` | 同左 |
| `ui_gallery` | `ui_gallery`, `ui-gallery`, `gallery` | 同左 |
| `ui_document_gallery` | `ui_document_gallery`, `ui-document-gallery`, `document_gallery`, `document-gallery`, `declarative_gallery` | 同左 |
| `ui_generated_acceptance` | `ui_generated_acceptance`, `ui-generated-acceptance`, `generated_acceptance` | 同左 |
| `ai_login_reference` | `ai_login_reference`, `ai-login-reference`, `image_login` | 不支持 |
| `sample_scene` | `sample_scene`, `sample-scene`, `sample` | 同左 |
| `robot_sync_scene` | `robot_sync_scene`, `robot-sync-scene`, `robot` | 同左 |
| `fangyuan_home` | `fangyuan_home`, `fangyuan-home`, `fangyuan` | 同左 |
| `fangyuan_player_preview` | `fangyuan_player_preview`, `fangyuan-player-preview`, `fangyuan_player`, `fangyuan-player`, `fangyuan_avatar`, `fangyuan-avatar` | `fangyuan_player_preview`, `fangyuan-player-preview`, `player_preview`, `player-preview` |
...
游戏内 `UiAuditPlugin` 负责可靠地执行 UI 状态控制、滚动、截图和 metadata 采集；外部 runner 负责多页面、多分辨率、多设备和多轮修复。远程模式下，命令通道、任务状态和 artifact 回收由 `docs/调试/远程调试控制机制.md` 约定。
...
- 清理旧产物时只删除 `summary/ui-audit/<run-id>/` 或 `summary/ui-audit/`，不要删除 `summary/.gitkeep`，也不要把 清单 归档流程混入 runner 清理。
- 修复 UI 时优先保持局部变更，避免为单一分辨率破坏通用控件。
- 自动修复必须保留最大迭代次数和失败出口。

## 视觉比较边界

游戏内 runner 负责进入页面、采集稳定截图和 runtime metadata，不负责实现视觉 diff。外部 `ui-visual-audit` 已提供可独立调用的 `compare`、`normalize-align`、`analyze-diff`、`audit-regions`、`audit-semantics` 和 aggregate gate；`analyze-ai` 还提供受严格输入、隐私裁切、凭据和预算约束的在线视觉分析 adapter。它们的结果可以作为 runner 闭环的 comparison/analysis artifact，但不能替代游戏内页面路由或 Android OS 截图。

当前仍未实现的是任意用户参考图的在线生成链路、完整的代码版与声明式 document 自动逐项对照，以及真实 Android 远程截图和系统 metadata 合同。
...
- 多窗口、多 client 同屏对比和跨设备设备编排；外部工具的单次视觉比较不等于多设备调度。
...
- 字体缩放矩阵。
- 完整 code-vs-document 自动对照和跨页面趋势归因。
- 历史趋势报告。
