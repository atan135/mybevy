...
普通表单、列表或后台工具页面优先使用现有组件和 `UiDocument`。声明式 Grid、card/frame、文字角色、控件 slot/state、TextInput 和高级图片的具体配置见 [界面生成式组件配置参考.md](界面生成式组件配置参考.md)。不要为了一个常规按钮重写一整页原生 Bevy 节点。
...
若使用能力矩阵中标为“允许直接使用 Bevy”的特性，按 [界面高保真视觉能力.md](界面高保真视觉能力.md) 的规则附加 `UiDirectBevyVisual` 及原因；不要借此绕过框架已有的公共控件、主题或动画能力。
