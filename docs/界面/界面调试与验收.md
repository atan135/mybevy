...
UI 调试能力由既有 `project/src/framework/ui/debug.rs` 和 `project/src/framework/devtools/live_preview/monitor.rs` 共同提供。前者维护 UI tree、layout bounds 等低层诊断，后者显示跨领域共享快照；两者都只读，不修改业务状态。
...
- `F8`：在监视器已打开且显式 Debug 授权时，把当前完整快照写入脱敏 JSON 内部 buffer；不会启动网络 listener 或写入生产 telemetry。未授权、Release 和 Android 模式 fail-closed。
...
当前 F3/F7 使用受控 Rust / Bevy UI，不依赖 `UiDocument`。多页监视器读取 `LivePreviewSnapshot`，页面为 Overview、UI、Player、Scene、Network、Performance 和 Timeline；UI 页继续保留 UI tree、过滤和 panel 高亮能力。共享快照、allowlist 脱敏、性能测量和未来外部显示端边界见 [实时状态预览与开发者监视器](../调试/实时状态预览与开发者监视器.md)。完整桌面链接、测试运行、窗口布局和场景人工验收仍属于阶段 10，不能用编译检查替代。
...
支持值见 [界面模式与面板层级.md](界面模式与面板层级.md)。
...
metadata 的 `image_snapshots`、`font_snapshots` 和 `visual_summary` 汇总图片 mode/status、样式 scope/variant、字体 role/status、效果、动画与控件状态；`visual_budget` 记录节点、解码图片 payload、render primitive、额外 effect draw、材质和 overdraw 的开发期估算。统计口径与 profile 阈值见 [界面安全区与视觉预算.md](界面安全区与视觉预算.md)。
...
大厅里的 `UI Gallery` 按钮现在进入 approved `UiDocument` 页面（`UiDocumentGallery`）；`ui_gallery` 直接启动别名仍保留为 Rust 对照页。两者共用 22 个 capture state 和滚动专题语义。声明式文档中的首批案例使用稳定 `gallery.pair.*` node ID，专题区使用 `gallery.section.*` node ID；游戏层把这些 node ID 适配到既有审计锚点，并在 `component_overlays` / `component_tooltip` 状态下确定性打开对应 document 控件，但不重新拥有 document 生命周期。
...
如果本机没有 `adb`，Android 设备级安装、渲染、触控、字体、图片切片和效果降级检查需要在有设备连接的环境完成。当前 2026-07-12 的开发环境即为此状态，真机项未完成；所需设备、API 和记录步骤见 [界面安全区与视觉预算.md](界面安全区与视觉预算.md)。
