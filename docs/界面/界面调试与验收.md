...
当前 F3/F7 仍是以 UI 诊断长文本为主的 Rust / Bevy UI，不是 `UiDocument`。计划中的跨领域实时预览会在保留现有快捷键、UI tree、过滤和 panel 高亮能力的基础上，将其扩展为读取共享只读状态快照的多页开发者监视器。目标页面包含 Overview、UI、Player、Scene、Network、Performance 和 Timeline；该目标、当前实现边界、敏感信息规则和未来外部显示端契约见 [实时状态预览与开发者监视器](../调试/实时状态预览与开发者监视器.md)。在对应 checklist 完成前，不应把统一快照或多页监视器描述为当前已实现能力。
