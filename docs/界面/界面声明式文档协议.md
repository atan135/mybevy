...
本文定义 `UiDocument` 的协议边界、版本兼容、安全模型、目录约定和首版资源预算。当前 Rust 数据模型、JSON Schema、验证器和 ECS runtime 以本文为协议依据；预览和 reload 的实现边界见 [界面声明式预览与热更新.md](界面声明式预览与热更新.md)，面向生成结果修订和组件配置检索的实践说明见 [界面生成式组件配置参考.md](界面生成式组件配置参考.md)。
...
参考图生成器属于协议外部 producer，不属于 `UiDocument` runtime。规划中的工具可以单向依赖 `project` 暴露的最小稳定 facade，复用 canonical JSON、Schema/语义校验、预算和开发期预览；`project`、Android 壳和正式构建不得反向依赖工具。provider、图片预处理、prompt、视觉分析、修复、评测、成本和日志实现不能注册进 `UiFrameworkPlugin` 或进入正式游戏依赖图。完整规划见 [界面参考图生成与正式包边界.md](界面参考图生成与正式包边界.md)。
...
图片预算使用 asset metadata 和解码后的尺寸计算，不信任 document 自报值。字体、图标、材质和效果必须来自 framework 注册表；document 不能内嵌二进制、base64、data URI 或 shader 参数 blob。阴影层、渐变 stop、图集 frame、平铺、材质和动画的细分上限由对应能力版本补充，但不得高于 `界面安全区与视觉预算.md` 的移动端策略。
