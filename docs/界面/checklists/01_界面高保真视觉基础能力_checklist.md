...
- 本清单是 `02_界面声明式描述与运行时生成_checklist.md` 和 `03_人工智能参考图生成界面_checklist.md` 的前置能力之一。
...
- [x] 只抽象真实参考图需要的能力，避免建立无法验证的全能样式系统。（验证：`界面高保真视觉能力.md` 的 11 类能力矩阵限定受支持/Direct Bevy/暂不支持边界，九宫格、效果、材质和动画均采用受限配置与显式拒绝）
...
- [x] 每个阶段独立实现、独立验证、独立提交，并同步检查 `docs/界面/`。（验证：`db95cd8` 至 `982a32d` 共 10 个顺序独立 `feat(ui)` 提交分别对应阶段 1-10，各阶段均记录测试/截图并同步 UI 文档）
...
- [x] 为每种能力定义“框架支持、允许直接使用 Bevy、暂不支持”三种状态及判定标准。（验证：`project/src/framework/ui/visual.rs:40` 定义三态模型，`docs/界面/界面高保真视觉能力.md` 记录判定顺序、矩阵和失败边界）
...
- [x] 更新 `docs/界面/界面组件功能与使用.md` 或新增对应框架说明，记录能力矩阵和非目标。（验证：新增 `docs/界面/界面高保真视觉能力.md`，并由 `docs/界面/README.md`、组件、限制和调试文档交叉引用）
...
- [x] 明确当前 Bevy 文本能力无法表达的字距、复杂富文本或排版效果，并提供资源化文字图片的受控替代方案。（验证：`docs/界面/界面主题字体与国际化.md` 和 `界面当前限制.md` 记录边界；`UiRasterizedTextSpec` 校验 ui 资源路径、可访问文案、i18n key 和来源，非法路径在 AssetServer 注册前失败）
...
- [x] 更新 `docs/界面/`、`docs/资源工作流.md` 和 `docs/引擎入门使用文档.md` 中受影响的说明。（验证：新增 `docs/界面/界面安全区与视觉预算.md`，并同步 UI README、响应式、限制、调试、高保真、效果、assets workflow 与 getting started；文档明确 fixture/估算不冒充真机或 GPU 实测）
...
- [x] Android 安全区有真实设备验证记录，或明确记录尚未满足的设备阻塞条件。（验证：满足后半条件；`ADB_NOT_FOUND` 已记录，`界面安全区与视觉预算.md` 列出 API 31+ 设备、cutout/导航/IME/旋转恢复与视觉触控补验清单，阶段真机执行项保持 `[ ]`）
...
- [x] `docs/界面/` 与实际公共 API、限制和资源工作流一致。（验证：UI README 与高保真、图片、字体、图标、样式、效果、动画、组件、响应式、调试、限制及安全区/预算文档已随阶段同步；assets workflow/getting started 明确 LFS、预算口径、Android 构建和真机边界）
