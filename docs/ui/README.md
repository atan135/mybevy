...
- [UI参考图生成与正式包边界.md](UI参考图生成与正式包边界.md)：独立 AI 生成工具的输入契约、生成期 staging、受控晋升、Rust 适配限制和正式桌面/Android 包隔离边界。Stage 1-2 已实现任务/依赖边界检查与离线 provider 基础；在线适配、图片预处理、生成和晋升尚未实现。
