...
- [UI参考图生成与正式包边界.md](UI参考图生成与正式包边界.md)：独立 AI 生成工具的输入契约、生成期 staging、受控晋升、Rust 适配限制和正式桌面/Android 包隔离边界。Stage 1-4 已实现任务/依赖边界、离线 provider、图片预处理和结构化分析协议；在线适配、文档生成和晋升尚未实现。
