...
- [UI参考图生成与正式包边界.md](UI参考图生成与正式包边界.md)：独立 AI 生成工具的输入契约、生成期 staging、受控晋升、Rust 适配限制和正式桌面/Android 包隔离边界。Stage 1-8 已实现离线 provider、预处理、分析、规划、素材策略、结构化文档、有限修复和独立预览；在线适配、自动生成落盘和晋升尚未实现。
