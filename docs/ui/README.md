...
- [UI参考图原版复刻工作流.md](UI参考图原版复刻工作流.md)：将参考图拆分为背景、主体、清理后面板和真实交互控件的高保真落地、预览与验收流程。
...
- [UI安全区与视觉预算.md](UI安全区与视觉预算.md)：Android WindowInsets 生产桥接、桌面 fixture、响应式综合验收和开发期预算口径。
...
- [UI声明式业务界面迁移基线.md](UI声明式业务界面迁移基线.md)：正式业务 View 的宿主边界、资源/Rust 判定、15 个页面盘点、例外、迁移指标与生产更新缺口。

## 设计方案

- [UI参考图生成与正式包边界.md](UI参考图生成与正式包边界.md)：独立 AI 生成工具的输入契约、生成期 staging、离线端到端 fixture、受控晋升、正式游戏路由适配和桌面/Android 包隔离边界。Stage 1-11、`generate-fixture` 与显式 `promote` 已实现；在线 provider/OCR/图片生成适配仍未实现。
