...
参考图生成器是独立 `tools/ui-generation/` 开发工具，不是 `UiDocumentPreviewPlugin` 的子模块。Stage 8 的 `preview-document` 命令会先在工具进程复验 staging JSON 和 packaged 资源，再启动 `project` 的 standalone binary，复用本页已有的完整校验、事务 reload、资源预检和截图。该 binary 只在显式 `ui-document-preview-tool` feature 下编译，并由 Cargo `required-features` 隔离；默认桌面、Android `--lib` 和正式 `UiFrameworkPlugin` 注册不包含该入口，也不包含 provider、图片预处理、prompt、generator、修复或评测代码。
...
standalone 进程等待 runtime commit、文档声明图片/字体就绪、稳定帧和截图完成，输出严格 result JSON、PNG 和日志；缺失资源、加载失败、资源/截图 timeout 及目标已存在都有稳定失败。工具侧继续核对 canonical document hash、viewport、result 路径和 captured size，并在受限分配下完整解码静态 8-bit PNG；只有磁盘图片真实尺寸精确匹配 viewport 后才接受其 hash/长度，run bundle 对复制后的证据再次执行相同校验。
...
只有显式 `promote` 流程可以在完整校验、人工批准、目标所有权、许可证和冲突检查后，把 `UiDocument` JSON、授权资源及封闭模板生成的 owner/route/registration 审阅声明写入正式目录。该命令已实现，并要求调用者重复 dry-run plan 的精确 hash；晋升后的页面仍须由游戏层显式 route lifecycle 通过 closed adapter 加载。验收样例 `generated.acceptance_fixture` 使用 `ui_generated_acceptance` 路由；详细约定见 [UI参考图生成与正式包边界.md](UI参考图生成与正式包边界.md)。
