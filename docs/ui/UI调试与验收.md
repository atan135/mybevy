...
$env:MYBEVY_UI_AUDIT_STATES="top,middle,bottom"
$env:MYBEVY_UI_AUDIT_EXIT_ON_FINISH="1"
cargo run -- --window-profile phone-small --window-scale 50%
```

输出目录会包含 `manifest.json`、`report.md`、`screenshots/` 和 `metadata/`。`ui_gallery` 已有滚动 recipe，会覆盖 `top`、`middle`、`bottom`；其他页面默认只覆盖 `initial`。
...
- 有滚动 recipe 的 `ui_gallery` 覆盖 `top`、`middle`、`bottom`，并记录 `scroll_target_id = ui_gallery.main`。
