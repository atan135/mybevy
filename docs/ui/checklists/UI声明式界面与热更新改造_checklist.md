...
- [x] checklist 全部完成后转移到 `docs/ui/checklists/` 归档，并同步更新 `docs/ui/`、`docs/assets-workflow.md`、`docs/bevy-getting-started.md` 和 `CLAUDE.md` 中受影响的约定。（验证：阶段 7/8/15 已同步资源工作流、UI 文档、上手文档和 CLAUDE；最终文件归档到 `docs/ui/checklists/`）
...
- 验证记录：核对 `AppUiMode` 固定为 15 个 screen，盘点表为 15 行；`git diff --check` 通过（仅 Git LF/CRLF 提示），改动仅位于 `docs/ui/`，本阶段无需构建或测试。

- [x] 定义并记录 `UiDocument View`、`Game Host`、`UiUpdateBundle`、`UiUpdateClient` 和 AI authoring/promotion 的职责与单向依赖。（审核：`docs/ui/UI声明式业务界面迁移基线.md` 第 1 节定义职责、禁止项和依赖图。）
...
- [x] 更新 UI 架构和当前限制文档，使“开发期 reload”和“生产用户端热更新”使用不同术语。（审核：`docs/ui/UI声明式预览与热更新.md`、`UI当前限制.md` 和 `UI框架整体架构.md` 明确 production bundle/client 未实现，Release/Android 不使用 file watch。）
- [x] 复核本阶段只修改文档和清单，运行 `git diff --check` 并确认路径正确。（验证：`git diff --check` 通过；`git status --short` 仅显示 `docs/ui/` 改动，checklist 位于被忽略的 `summary/`。）
...
- [x] 更新 Schema、canonical golden、协议文档和完整有效/无效 fixture。（验证：`ui_document.v1.schema.json`、`binding_typed_values.v1.{json,canonical.json}`、`binding_controls.v1.json`、两份 invalid fixture 与 `docs/ui/UI声明式文档协议.md` 同步更新。）
...
- [x] 更新 promotion、安全边界和人工审核文档，明确“AI 引用既有能力”不等于“AI 创建业务实现”。（验证：更新 `docs/ui/` 的限制、迁移基线、协议和生成/正式包边界。）
...
- [x] 更新页面迁移清单和指标，明确尚未迁移页面及原因。（验证：`docs/ui/UI声明式业务界面迁移基线.md` 将 Lobby 标为正式声明式并更新为已声明式 5、普通业务待迁移 1，剩余为阶段 14 的 audio settings）
...
- [x] 更新设置/UI 文档和页面迁移指标。（验证：`UI声明式业务界面迁移基线.md` 更新为已声明式 6、普通业务待迁移 0 并新增 Audio Settings 宿主语义；`UI组件功能与使用.md` 链接正式 Slider/Toggle 用法）
...
- [x] `docs/ui/`、资源工作流、上手文档和 `CLAUDE.md` 已与最终架构一致，不再把开发期 reload 描述为用户端生产热更新。（验证：相关文档明确区分 desktop Debug preview/watch 与显式配置的签名 production `UiUpdateClient`，并记录默认应用/Android 尚缺的发布接线）
- [x] 本 checklist 已从 `summary/` 转移到 `docs/ui/checklists/` 归档并纳入最终提交。（验证：归档路径 `docs/ui/checklists/UI声明式界面与热更新改造_checklist.md`）
