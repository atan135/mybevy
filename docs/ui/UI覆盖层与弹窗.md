...
覆盖层实现位于 `project/src/game/ui/overlays/`，并由 `UiRouterPlugin` 和 `UiPanelPlugin` 统一调度。当前覆盖层包括 Toast、Loading、Confirm modal 和 Floating panel。

## Toast

Toast 使用 `UiRouteCommand::ShowToast(UiToast)` 打开，不进入 Panel Manager。
...
- 固定面板 id 是 `UiPanelId::GlobalLoading`。
...
- 固定面板 id 是 `UiPanelId::ConfirmModal`。
...
- 按下动作按钮会发送 `UiModalResult { id, action }`，然后关闭 Confirm。
- 入场有 alpha 动画；关闭时当前直接 despawn，没有出场动画。
...
- id 由 `UiFloatingPanel.id` 指定，例如 `GalleryFloating`。
...
通过 Panel Manager 打开的覆盖层会带当前 `AppUiMode` 作为 `owner_mode`。页面模式切换时，路由系统先发送 `CloseAllForMode(current_mode)`，清理当前模式拥有的覆盖层，再进入目标模式。

## 动画

当前 UI 动画核心在 `project/src/game/ui/core/animation.rs`。覆盖层主要使用 alpha 动画：
