...
本文冻结“参考图 -> `UiDocument` -> 正式游戏页面”流程的工程边界。仓库当前已提供 Stage 1-2 `tools/ui-generation/` 独立工具、输入契约、工作目录规划、provider 无关调用协议以及离线 fixture/mock；在线供应商适配、图片预处理、文档生成、预览接入和 `promote` 命令仍未实现。
