...
每次修复 iteration 都必须在允许根内采集 before/after SHA-256 文件快照，并以 `created`、`modified`、`deleted` 三类结构化 diff 保存。路径解析会拒绝绝对路径、`..`、symlink/reparse point 和越过允许根的输出。Runner 不得对用户当前工作树调用 reset、clean、覆盖 checkout、worktree remove 或递归删除。
